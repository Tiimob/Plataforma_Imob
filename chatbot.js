...
(function () {
  ...
  function categoriaDetectada(tokens, perguntaNormalizada = '') {
    ...
    // Regra interna: a gestão da carteira de clientes é feita exclusivamente no Hypnobox.
  ...
  function buscarResposta(pergunta) {
    ...
    const somenteHypnobox = categoriaPrincipal === 'Hypnobox' && ehDuvidaGestaoClientes(perguntaNormalizada, tokensPergunta);
  ...
  function criarBolha(tipo) {
    ...
    if (tipo === 'usuario') {
      // A mensagem do usuário é exibida sem avatar ou abreviação.
  ...
  function adicionarIndicadorAnalise() {
    ...
    bolha.setAttribute('aria-label', 'Analisando sua dúvida');

    const texto = document.createElement('span');
    texto.textContent = 'Analisando sua dúvida';
  ...
  async function processarPergunta(pergunta) {
    ...
    try {
      await aguardar(TEMPO_RESPOSTA_MS);
      indicador?.remove();
